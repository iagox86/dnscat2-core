...
# See: LICENSE.md
##
# As of July/2018, this class is getting a major revamp to fit with the new
# dnscat2-core standards.
#
# The following are notes on how this needs to work...
#
# * It abstractly handles all elements of the dnscat2 lifecycle (create,
#   maintain, and destroy sessions)
# * Other libraries can subscribe to different events (on_create, on_destroy,
#   etc)
# * Sessions are assigned a sequential number for internal usage (callbacks),
#   and a random number for on-the-wire communication - they are translated here
# * Everything works at the Packet level, not data - this class should never
#   worry about a stream of bytes
# * Must handle the encryption/decryption of packets
# * Must handle receiving ping responses
#
# There are two ways for data to come and go:
# * From the application (new data or commands coming from the application)
#   * This works as a stream of bytes: they come in via the feed() function,
#     and go out via a callback
# * From the network (a new packet has arrived)
#   * Packets arrive via a tunnel_driver, which is passed in to initialize()
#   * The Session class "subscribes" to the tunnel driver, which feeds it
#     packets (the tunnel_driver needs to create a packet)
#   * Session will return a packet to be sent back, or nil if there is
#     an error and the packet should be nack'd (NXDomain or ServFail)
#
# Callbacks:
# * packet_received, packet_sent (for logging)
# * encrypted_packet_recieved, encrypted_packet_sent (for logging encrypted
#   packets)
# * data_received, data_sent (for actual processing)
# * session_started, session_ended (for lifecycle management)
# * keepalive (fires occasionally, in lockstep with the delay)
##
...
require 'dnscat2/core/libs/logger'
...
module Dnscat2
  module Core
    class Session
      ...
      def initialize(isn:nil, tunnel_driver:)
        @state = STATE_NEW
        @their_seq = 0

        @my_seq = rand(0xFFFF)
        # Only let the ISN be set if we're TESTING
        if(!isn.nil? && TESTING)
          @my_seq = isn
        end
        ...
        @encryptor = Encryptor.new(preshared_secret: 'TODO')
      end
