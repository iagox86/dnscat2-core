...
#
# More details here:
# https://github.com/iagox86/dnscat2/blob/master/doc/protocol.md#dns-transport-protocol
##
  ...
  module Core
    module TunnelDrivers
      class DNS
        ...
        public
        def initialize(s:, prefix:, domains:, controller:, host:"0.0.0.0", port:53)
          @l = SingLogger.instance
          @l.debug("TunnelDrivers::DNS New instance! s = #{s}, prefix = #{prefix}, domains = #{domains}, controller = #{controller}, host = #{host}, port = #{port}")

          @prefix = prefix
          @domains = domains

          @nesser = Nesser::Nesser.new(s:s, logger: @l, host:host, port:port) do |transaction|
            _handle_transaction(transaction)
          end
        end
