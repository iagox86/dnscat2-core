    ...
    module TunnelDrivers
      module DNS
        class NameHelperTest < ::Test::Unit::TestCase
          ...
          def test_max_length()
            63.step(1, -1) do |i|
              _, p1, p2 = NameHelper.new(tag: nil, domain: nil, max_subdomain_jitter: 0, max_subdomain_length: i).max_length
              puts("#{i} => #{p1} #{p2}")
            end
            ...
            1.upto(250) do |i|
              _, p1, p2 = NameHelper.new(tag: "a" * i, domain: nil, max_subdomain_jitter: 0, max_subdomain_length: 63).max_length
              puts("#{i} => #{p1} #{p2}")
            end
            ...
            # Prepending 'a.' means we have two less bytes, so (255 - 4 periods - 2 bytes) / 2 characters/byte => 124
            #assert_equal(124, NameHelper.new(tag: 'a', domain: nil, max_subdomain_jitter: 0).max_length)

            # Prepending 'aa.' means we have two less bytes, so (255 - 4 periods - 3 bytes) / 2 characters/byte => 124
            #assert_equal(124, NameHelper.new(tag: 'aa', domain: nil, max_subdomain_jitter: 0).max_length)

            # (255 - 4 - 4) / 2
            #assert_equal(123, NameHelper.new(tag: 'aaa', domain: nil, max_subdomain_jitter: 0).max_length)

            # (255 - 4 - 5) / 2
            #assert_equal(123, NameHelper.new(tag: 'aaaa', domain: nil, max_subdomain_jitter: 0).max_length)

            # (255 - 4 - 6) / 2
            #assert_equal(122, NameHelper.new(tag: 'aaaaa', domain: nil, max_subdomain_jitter: 0).max_length)
